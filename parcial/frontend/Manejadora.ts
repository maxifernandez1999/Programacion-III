...
// Informar por consola y alert el mensaje recibido.
        public static AgregarProductoJSON(){
            ...
            var param:string = `nombre=${nombre}&correo=${origen}`;
        ...
        public static MostrarProductosJSON(){
            ...
            (mensaje:string) => {
                ...
                for (let index = 0; index < arrayProductosJSON.length; index++) {
                    var json:string = `<tr><td>${arrayProductosJSON[index].nombre}</td><td>${arrayProductosJSON[index].origen}</td><td>${arrayProductosJSON[index].id}</td><td>${arrayProductosJSON[index].codigoBarra}</td><td>${arrayProductosJSON[index].pathFoto}</td><td>${arrayProductosJSON[index].precio}</td></tr>`;
...
// Informar por consola el mensaje recibido y mostrar el listado en la página (div id='divTabla').
        public MostrarProductosEnvasados(){
            let ajax : Ajax = new Ajax();
