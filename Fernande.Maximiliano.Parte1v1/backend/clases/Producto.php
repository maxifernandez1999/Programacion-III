<?php
    ...
    class Producto 
    {
        ...
        private static function MasPopulares($masPopulares){
            $count = array();
            ...
            $nombres = "";
            foreach ($masPopularesNombres as $key => $value) {
        ...
        public static function VerificarProductoJSON($nombre,$origen){
            $arrayjson = Producto::TraerJSON('./archivos/productos.json');
            $stdClass = new stdClass();
            $retorno = true;
            $masPopulares = array();
            ...
            if ($retorno == false) {
                $stdClass->exito = false;
                $stdClass->mensaje = "El/los producto/os con mas apariciones es/son: ".Producto::MasPopulares($masPopulares);
            }
            return json_encode($stdClass);
        }
