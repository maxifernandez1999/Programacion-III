...
class Auto
{
	...
	public function AgregarAuto(Request $request, Response $response, array $args){
		$objJSON = isset($request->getParsedBody()['auto']) ? json_decode($request->getParsedBody()['auto']) : null;
	...
	public function TraerAutos(Request $request, Response $response, array $args): Response {
		$stdclass =new stdClass();
		$AllAutos = self::TraerTodosDB();
		if($AllAutos!=null){
			$stdclass->exito = true;
			$stdclass->mensaje = "autos";
			$stdclass->dato = json_encode($AllAutos);
			$stdclass->status = 200;
			$response->getBody()->write(json_encode($stdclass));
		}else{
			$stdclass->exito = false;
	...
	public function EliminarAuto(Request $request, Response $response, array $args){
		...
		if($idAuto != null && $data != null){
			if($data->perfil == "propietario"){
				...
				$datos->exito = false;
				$datos->mensaje = $data->nombre."No propietario";
				$datos->status = 418;
			}
	...
	public function ModificarAuto(Request $request, Response $response, array $args){
		...
		if($idAuto != null && $data != null){
			if($data->perfil == "encargado"){
				...
				$datos->exito = false;
				$datos->mensaje = $data->nombre."No es encargado";
				$datos->status = 418;
			}
